#!/usr/bin/env node
/**
 * ingest-v6-docs-sectioned.js
 *
 * Reads FeathersJS v6 markdown documentation from docs/v6_docs/ and writes
 * structured JSON entries with SECTION-LEVEL CHUNKING into knowledge-base/docs/v6/.
 *
 ...
 *
 * Usage:
 *   node scripts/ingest-v6-docs-sectioned.js [--source <path>]
 *
...
const KB_DOCS_DIR = path.join(PROJECT_ROOT, 'knowledge-base', 'docs', 'v6');
 ...
 * Format: v6/category/filename#heading-slug
 */
...
function main() {
  console.log('');
  console.log('  🪶  FeathersJS MCP — v6 Documentation Ingester (Section-Level Chunking)');
  console.log('  ──────────────────────────────────────────────────────────────────────');
  ...
  if (!fs.existsSync(V6_DOCS_DIR)) {
    ...
    console.error(
      '    node scripts/ingest-v6-docs-sectioned.js --source /path/to/feathers-v6/website/content'
    );
    console.error('');
    process.exit(1);
  }
  ...
  for (const relPath of mdFiles) {
    ...
    const fileTitle = extractTitle(cleaned, `FeathersJS v6 — ${fileBasename}`);
  ...
  for (const [category, entries] of byCategory.entries()) {
    ...
    // Merge with existing entries if file exists (preserve non-v6 entries)
    let existing = [];
    ...
    console.log(`  📄  Wrote v6/${outFile}  (${entries.length} v6 entries)`);
    filesWritten++;
  }
  ...
  console.log(`  Files      : ${filesWritten} JSON file(s) written to knowledge-base/docs/v6/`);
  console.log('');
  console.log('  Next step — regenerate embeddings:');
  console.log('    npm run generate:embeddings');
  console.log('');
  console.log('  Then commit knowledge-base/docs/v6/');
  console.log('');
}
